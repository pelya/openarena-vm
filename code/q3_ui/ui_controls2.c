...
static bind_t g_bindings[] = 
{
	...
	{"+centerview",		"quick rotate",		ID_CENTERVIEW,	ANIM_IDLE,		K_END,			-1,		-1, -1},
...
static void Controls_DrawKeyBinding( void *self )
{
	...
	if (c)
	{
		...
		if (s_controls.waitingforkey)
		{
			...
			UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.89, "Waiting for new key ... ESCAPE to cancel", UI_SMALLFONT|UI_CENTER|UI_PULSE, colorWhite );
		}
		else
		{
			UI_DrawChar( x, y, 13, UI_CENTER|UI_BLINK|UI_SMALLFONT, text_color_highlight);
			UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.89, "ENTER or CLICK to change, BACKSPACE to clear", UI_SMALLFONT|UI_CENTER, colorWhite );
		}
	}
	else
	{
...
{
	UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.89, "Use Arrow Keys or CLICK to change", UI_SMALLFONT|UI_CENTER, colorWhite );
}
...
static void Controls_MenuInit( void )
{
	...
	s_controls.invertmouse.generic.name	     = "invert vertical aim";
