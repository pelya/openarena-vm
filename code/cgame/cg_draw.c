...
//		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
	...
	*/
	...
	CG_DrawStatusBarHead( 285 + CHAR_WIDTH*3 + TEXT_ICON_SPACE - 5, 480);

	if( cg.predictedPlayerState.powerups[PW_REDFLAG] ) {
		CG_DrawStatusBarFlag( 435 + CHAR_WIDTH*3 + TEXT_ICON_SPACE + ICON_SIZE, 480, TEAM_RED );
	} else if( cg.predictedPlayerState.powerups[PW_BLUEFLAG] ) {
		CG_DrawStatusBarFlag( 435 + CHAR_WIDTH*3 + TEXT_ICON_SPACE + ICON_SIZE, 480, TEAM_BLUE );
	} else if( cg.predictedPlayerState.powerups[PW_NEUTRALFLAG] ) {
		CG_DrawStatusBarFlag( 435 + CHAR_WIDTH*3 + TEXT_ICON_SPACE + ICON_SIZE, 480, TEAM_FREE );
	}
	...
	if ( cent->currentState.weapon ) {
		...
		if ( value > -1 ) {
			...
			if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
				...
				if ( icon ) {
					CG_DrawPic( 130 + CHAR_WIDTH*3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE, icon );
				}
			}
		}
	}
	...
	if (value > 0 ) {
		...
		CG_DrawField (435, 432, 3, value);
		trap_R_SetColor( NULL );
		// if we didn't draw a 3D icon, draw a 2D icon for armor
		if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
			CG_DrawPic( 435 + CHAR_WIDTH*3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE, cgs.media.armorIcon );
		}
        ...
        if(cgs.gametype == GT_HARVESTER)
        {
            ...
            if (value > 0 ) {
                    ...
                    if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
                            ...
                            CG_DrawPic( 480 + CHAR_WIDTH*3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE, handle );
                    }
...
static int CG_DrawPickupItem( int y ) {
	...
	if ( value ) {
		...
		if ( fadeColor ) {
			...
			CG_DrawPic( 230, y, ICON_SIZE, ICON_SIZE, cg_items[ value ].icon );
			CG_DrawBigString( ICON_SIZE + 238, y + (ICON_SIZE/2 - BIGCHAR_HEIGHT/2), bg_itemlist[ value ].pickup_name, fadeColor[0] );
			trap_R_SetColor( NULL );
		}
	}
